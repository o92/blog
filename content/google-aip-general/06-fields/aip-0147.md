...
若敏感信息对资源整体存在是*必需的*，数据 **should** 作为 [input-only 字段]({{< ref "aip-0203" >}}) 接受，没有对应的输出字段。因为资源要存在就必须有敏感数据，API 用户可以假定资源的存在意味着敏感数据已存储。例如：
...
若敏感信息在包含资源内是*可选的*，**should** 使用带 `_set` 后缀的 [output-only]({{< ref "aip-0203" >}}) 布尔字段，以表明敏感信息是否存在。例如：
